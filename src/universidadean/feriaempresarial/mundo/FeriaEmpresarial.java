/**
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ $Id: FeriaEmpresarial.java
 * 718 2008-10-03 21:07:55Z jua-gome $ Universidad Ean (Bogotá - Colombia) Programa de Ingeniería de
 * Sistemas Licenciado bajo el esquema Academic Free License version 2.1
 *
 * <p>Bloque de Estudios: Desarrollo de Software Ejercicio: Feria Empresarial Adaptado de: Proyecto
 * CUPI2 - UNIANDES ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 */
...
public class FeriaEmpresarial {
   ...
   * @param nNumeroPersonasAsistentes Es el número de personas de la empresa asistentes a la feria.
   *     nNumeroPersonasAsistentes > 0
   * @throws Exception <br>
   ...
   */
  public void ingresarEmpresaVisitante(String nNombreEmpresa, int nNumeroPersonasAsistentes)
      throws Exception {
   ...
   * <b>pre: </b>La lista de empresas y los puestos han sido inicializados<br>
   * <b>post: </b>Se ha adicionado una empresa expositora a la contenedora de empresas con los
   ...
   *
   * @param nNombreEmpresa Es el nombre de la empresa. nNombreEmpresa != null,nNombreEmpresa != ""
   * @param nNumeroPersonasExpositoras Es el número de personas de la empresa expositora .
   *     nNumeroPersonasExpositoras > 0
   ...
   *     5. Si el numero de personas expositoras no esta dentro del rango de personas permitidas en
   *     el puesto
   */
  public void ingresarEmpresaExpositora(
      String nNombreEmpresa, int nNumeroPersonasExpositoras, String zonaPuesto, int numeroPuesto)
      throws Exception {
    boolean encontrado = false;
    ...
    if (zonaPuesto.equals(Puesto.ZONA_NORTE)) {
      if (nNumeroPersonasExpositoras < MIN_PERSONAS_NORTE
          || nNumeroPersonasExpositoras > MAX_PERSONAS_NORTE) {
      ...
      if (nNumeroPersonasExpositoras < MIN_PERSONAS_ORIENTE
          || nNumeroPersonasExpositoras > MAX_PERSONAS_ORIENTE) {
        throw new Exception("Número de personas no permitidas para la zona");
      }
      ...
      if (nNumeroPersonasExpositoras < MIN_PERSONAS_SUR
          || nNumeroPersonasExpositoras > MAX_PERSONAS_SUR) {
        throw new Exception("Número de personas no permitidas para la zona");
      }
      ...
      if (nNumeroPersonasExpositoras < MIN_PERSONAS_OCCIDENTE
          || nNumeroPersonasExpositoras > MAX_PERSONAS_OCCIDENTE) {
        throw new Exception("Número de personas no permitidas para la zona");
      }
      ...
      if (nNumeroPersonasExpositoras < MIN_PERSONAS_CENTRO
          || nNumeroPersonasExpositoras > MAX_PERSONAS_CENTRO) {
        throw new Exception("Número de personas no permitidas para la zona");
      }
   ...
   * El puesto sugerido es aquel que primero puede albergar la cantidad de expositores y está
   * disponible<br>
   * El recorrido por zonas se hace en el siguiente orden: Norte, Oriente, Sur, Occidente, Centro.
   *
   * @param numeroExpositores > 0
   * @return Puesto sugerido para la cantidad de expositores. null si no hay puesto disponible que
   *     albergue a los expositores.
   */
  public Puesto sugerirPuesto(int numeroExpositores) {
    ...
    for (int i = 0; i < puestos.length && !encontro; i++) {
      Puesto puesto = puestos[i];
      if (!puesto.estaOcupado()) {
        if (numeroExpositores <= puesto.darNumMaximoPersonas()
            && numeroExpositores >= puesto.darNumMinimoPersonas()) {
   ...
   * <b>pre: </b> El arreglo de puestos tiene tamaño NUM_PUESTOS_NORTE + NUM_PUESTOS_SUR +
   * NUM_PUESTOS_ORIENTE + NUM_PUESTOS_OCCIDENTE + NUM_PUESTOS_CENTRO<br>
   * <b>post: </b> Los puestos han sido inicializados con sus valores de acuerdo a su zona de
   * ubicación<br>
   * Los puestos se llenan en el siguiente orden por zonas: Norte, Oriente, Sur, Occidente, Centro.
   * Cada zona se llena por número de puesto incrementálmente
   */
  private void crearPuestos() {
    ...
    for (int i = 0; i < NUM_PUESTOS_ORIENTE; i++) {
      puestos[i + NUM_PUESTOS_NORTE] =
          new Puesto(numPuesto, zona, MIN_PERSONAS_ORIENTE, MAX_PERSONAS_ORIENTE);
      numPuesto++;
    }
    ...
    for (int i = 0; i < NUM_PUESTOS_SUR; i++) {
      puestos[i + NUM_PUESTOS_NORTE + NUM_PUESTOS_ORIENTE] =
          new Puesto(numPuesto, zona, MIN_PERSONAS_SUR, MAX_PERSONAS_SUR);
      numPuesto++;
    }
    ...
    for (int i = 0; i < NUM_PUESTOS_OCCIDENTE; i++) {
      puestos[i + NUM_PUESTOS_NORTE + NUM_PUESTOS_ORIENTE + NUM_PUESTOS_SUR] =
          new Puesto(numPuesto, zona, MIN_PERSONAS_OCCIDENTE, MAX_PERSONAS_OCCIDENTE);
      numPuesto++;
    }
    ...
    for (int i = 0; i < NUM_PUESTOS_CENTRO; i++) {
      puestos[
              i
                  ...
                  + NUM_PUESTOS_SUR
                  + NUM_PUESTOS_OCCIDENTE] =
          new Puesto(numPuesto, zona, MIN_PERSONAS_CENTRO, MAX_PERSONAS_CENTRO);
      numPuesto++;
    }
  }
  ...
  public String metodo1() {
    Map<String, Integer> mapaDeZonas = obtenerMapaDeZonas();

    int numeroMayorEntreZonas = (Collections.max(mapaDeZonas.values()));
    for (Map.Entry<String, Integer> entry : mapaDeZonas.entrySet()) {
      if (entry.getValue() == numeroMayorEntreZonas) {
        return entry.getKey();
      }
    }
  ...
  private Map<String, Integer> obtenerMapaDeZonas() {

    Map<String, Integer> mapaDeZonas = new HashMap();

    mapaDeZonas.put(Puesto.ZONA_NORTE, this.obtenerNumeroDeExpositoresPorZona(Puesto.ZONA_NORTE));
    mapaDeZonas.put(Puesto.ZONA_SUR, this.obtenerNumeroDeExpositoresPorZona(Puesto.ZONA_SUR));
    mapaDeZonas.put(Puesto.ZONA_CENTRO, this.obtenerNumeroDeExpositoresPorZona(Puesto.ZONA_CENTRO));
    mapaDeZonas.put(
        Puesto.ZONA_ORIENTE, this.obtenerNumeroDeExpositoresPorZona(Puesto.ZONA_ORIENTE));
    mapaDeZonas.put(
        Puesto.ZONA_OCCIDENTE, this.obtenerNumeroDeExpositoresPorZona(Puesto.ZONA_OCCIDENTE));

    return mapaDeZonas;
  }

  private int obtenerNumeroDeExpositoresPorZona(String zona) {
    ArrayList<Puesto> puestosEncontrados = this.encontrarPuestosPorZona(zona);
    int expositores = 0;
    for (Puesto puesto : puestosEncontrados) {
      try {
        expositores += this.obtenerNumeroDeExpositoresPorPuesto(puesto);
      } catch (Exception e) {
      }
    }
  ...
  private int obtenerNumeroDeExpositoresPorPuesto(Puesto puesto) throws Exception {
    if (!puesto.estaOcupado()) return 0;
    return (this.encontrarEmpresaPorNombre(puesto.darNombreEmpresa()))
        .darNumeroPersonasAsistentes();
  }

  private Empresa encontrarEmpresaPorNombre(String nombre) throws Exception {
    for (Empresa empresa : this.empresas) {
      if (empresa.darNombre() == nombre) {
        return empresa;
      }
    }
  ...
  private ArrayList<Puesto> encontrarPuestosPorZona(String zona) {
    ArrayList<Puesto> puestosEncontrados = new ArrayList<Puesto>();
    for (Puesto puesto : this.puestos) {
      if (puesto.darZona() == zona) {
        puestosEncontrados.add(puesto);
      }
    }
    return puestosEncontrados;
  }
  ...
  public String metodo2() {
    double porcentaje = this.porcentajeOcupacionPorExpositores();
  ...
  private double porcentajeOcupacionPorExpositores() {
    Map<String, Integer> mapaDeZonas = this.obtenerMapaDeZonas();
    int sumaTotalExpositores = mapaDeZonas.values().stream().reduce(0, Integer::sum);
    int MAXIMO_EXPOSITORES =
        MAX_PERSONAS_CENTRO * 8
            + MAX_PERSONAS_NORTE * 4
            + MAX_PERSONAS_SUR * 4
            + MAX_PERSONAS_ORIENTE * 3
            + MAX_PERSONAS_OCCIDENTE * 3;

    return (sumaTotalExpositores * 100) / MAXIMO_EXPOSITORES;
  }
}
